...
// Компонент: описывает протокол TCL/Ballu и 65-байтовый ответ кондиционера.
...
class TCLClimate : public climate::Climate, public uart::UARTDevice, public PollingComponent {
 public:
  explicit TCLClimate(uart::UARTComponent *parent) : UARTDevice(parent) {}
  ...
  // Ответ Ballu/TCL: 65 байт, включая четыре дополнительных байта 60–63 и XOR в байте 64.
  union get_cmd_resp_t {
    struct {
      ...
      // Дополнительные байты именно твоей 65-байтовой ревизии Ballu.
      uint8_t byte_60;
      uint8_t byte_61;
      uint8_t byte_62;
      uint8_t byte_63;
  ...
  // Команда управления TCL/Ballu: штатный пакет длиной 35 байт.
  union set_cmd_t {
    struct {
      uint8_t header;
      uint8_t byte_1;
  ...
  uint8_t set_cmd_base[35] = {
      0xBB, 0x00, 0x01, 0x03, 0x1D, 0x00, 0x00, 0x64, 0x03,
  ...
  void set_beeper_enabled(bool enabled);
 ...
 private:
  ...
  void send_current_state_command_();
